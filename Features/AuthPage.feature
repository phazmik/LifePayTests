...
Сценарий: При введении не зарегистрированного номера телефона выводится соответствующее сообщение
	Допустим Открыта страница авторизации LifePay 
	Когда поле ввода Номер телефона заполнено значением '+7(911)111-11-12'
	Тогда отображается предупреждение 'Номер не зарегистрирован' в поле Номер телефона 
...
Сценарий:  Отображение предупреждения поля Номер телефона при введении пароля длиной менее 6 знаков
	Допустим Открыта страница авторизации LifePay
	Когда поле ввода Пароль заполнено значением '@1234'
	Тогда отображается предупреждение 'Значение должно быть не менее 6 знаков' в поле Пароль 
